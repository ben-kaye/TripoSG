...
def load_image(img_path, bg_color=None, rmbg_net=None, padding_ratio=0.1):
    ...
    scale = 2000 / height if height > width else 2000 / width
    if scale < 1:
        new_size = (int(width * scale), int(height * scale))
        img = cv2.resize(img, new_size, interpolation=cv2.INTER_AREA)

    if img.dtype != "uint8":
        img = (img * (255.0 / np.iinfo(img.dtype).max)).astype(np.uint8)
    ...
    if num_channels == 1:
        ...
        _b, _g, _r, alpha = cv2.split(img)
        if not is_valid_alpha(alpha):
            alpha = None
        else:
            alpha_gpu = torch.from_numpy(alpha).unsqueeze(0).cuda().float() / 255.0
    else:
        return f"invalid image: channels {num_channels}"

    rgb_image_gpu = torch.from_numpy(rgb_image).cuda().float().permute(2, 0, 1) / 255.0
    if alpha is None:
        ...
        resize_transform = transforms.Resize(
            (rgb_image_gpu.shape[1], rgb_image_gpu.shape[2]), antialias=True
        )
