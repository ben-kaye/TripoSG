...
def run_triposg_scribble(
    ...
    outputs = pipe(
        ...
        hierarchical_octree_depth=8,  # 256 resolution for faster inference
    ).samples[0]
...
if __name__ == "__main__":
    ...
    snapshot_download(
        repo_id="VAST-AI/TripoSG-scribble", local_dir=triposg_scribble_weights_dir
    )

    # init tripoSG pipeline
    pipe: TripoSGScribblePipeline = TripoSGScribblePipeline.from_pretrained(
        triposg_scribble_weights_dir
    ).to(device, dtype)
